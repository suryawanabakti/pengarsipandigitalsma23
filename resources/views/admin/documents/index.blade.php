        ...
        <div style="padding: 20px; border-bottom: 1px solid var(--border);">
            <form action="{{ route('documents.index') }}" method="GET" style="display: flex; gap: 15px; flex-wrap: wrap;">
                <div style="flex: 1; min-width: 200px;">
                    <input type="text" name="search" class="form-control" placeholder="Cari judul atau nomor surat..."
                        ...
                        <tr>
                            ...
                            <td>
                                @foreach ($doc->tags as $tag)
                                    <span
                                        style="font-size: 10px; background: #e2e8f0; padding: 2px 6px; border-radius: 4px; margin-right: 2px;">
                                        {{ $tag->name }}
                                    </span>
                                @endforeach
                            </td>
                            <td>
                                <div style="display: flex; gap: 8px;">
                                    <a href="{{ route('documents.show', $doc) }}" class="btn"
                                        style="padding: 6px; font-size: 12px; background: var(--primary); color: white;"
                                        title="Lihat Detail">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="{{ route('documents.download', $doc) }}" class="btn"
                                        style="padding: 6px; font-size: 12px; background: var(--info); color: white;"
                                        title="Download">
                                        <i class="fas fa-download"></i>
                                    </a>
                                    @if ($doc->status !== 'disetujui' && (auth()->user()->role->name == 'Admin' || $doc->uploaded_by == auth()->id()))
                                        <a href="{{ route('documents.edit', $doc) }}" class="btn"
